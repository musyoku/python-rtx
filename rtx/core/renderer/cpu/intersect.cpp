...
namespace cpu {
    float intersect_sphere(
        const glm::vec3f& center,
        const float radius,
        const std::unique_ptr<Ray>& ray)
    {
        const glm::vec3f oc = ray->_origin - center;
        ...
        const std::unique_ptr<Ray>& ray)
    {
        ...
        const glm::vec3f edge_ba = vb - va;
        const glm::vec3f edge_ca = vc - va;

        const glm::vec3f h = glm::cross(ray->_direction, edge_ca);
        const float a = glm::dot(edge_ba, h);
        if (a > -eps && a < eps) {
            return -1.0f;
        }
        const float f = 1.0f / a;
        const glm::vec3f s = ray->_origin - va;
        const float u = f * glm::dot(s, h);
        if (u < 0.0f || u > 1.0f) {
            return -1.0f;
        }
        const glm::vec3f q = glm::cross(s, edge_ba);
